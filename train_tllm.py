...
def train(configs, exp_name, g):
    ...
    if configs["train"]:
        ...
        for train_sample_step, batch in enumerate(t:=tqdm.tqdm(train_loader)):
            question, answer_tokens, tactile_frames, tactile, question_type, question_step = batch
            answer_tokens = answer_tokens.to(device)
            outputs, _ = model(question=question, tactile_frames=tactile_frames, answer_tokens=answer_tokens)
    ...
    if configs["val"]:
        ...
        with torch.no_grad():
            for val_sample_step, batch in enumerate(tqdm.tqdm(val_loader)):
                ...
                question, answer_tokens, tactile_frames, tactile, question_type, question_step = batch
                answer_tokens = answer_tokens.to(device)
                outputs, question_embeds = model(question=question, tactile_frames=tactile_frames, answer_tokens=answer_tokens)
