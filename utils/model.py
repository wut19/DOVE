...
class CLIPClassifier(nn.Module):
    ...
    def forward(self, vision_features):
        ...
        material_preds = self.material_fc(vision_features)
...
class TactileCLIP(nn.Module):
    ...
    def forward(self, tactile_frames, texts, attention_masks):
        # embedding
        b, c, h, w = tactile_frames.shape # (b, c, h, w)
        vision_outputs = self.clip_model.vision_model(tactile_frames)
        pooled_output = vision_outputs.pooler_output # (b, patch_embed_size)
        # 
        vision_features = pooled_output
        ...
        if texts is not None:
            ...
            # get logits
            logit_scale = self.clip_model.logit_scale.exp()
            cosine_sim = torch.matmul(text_features, vision_features.t())
            logits_per_text = cosine_sim * logit_scale
            logits_per_image = logits_per_text.t()
        else:
            text_features = None
